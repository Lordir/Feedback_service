...
def test_register(client, app):
    ...
    with app.app_context():
        added_user = Users.query.filter_by(username='test2').first()

        assert added_user is not None
        assert added_user.username == "test2"
...
def test_review_update(client, auth):
    auth.login()
    response = client.get('/review/3/update')
    assert "test" in response.data.decode()

    client.post('/review/3/update',
                data={"title": "test99", "rating": 3, "review_text": "", "category_id": 3, "user_id": 4},
                follow_redirects=True)

    response2 = client.get('/review/3/update')
    assert "test99" in response2.data.decode()

    "При вводе некорректных данных ничего не обновится"
    client.post('/review/3/update',
                data={"title": "", "rating": 3, "review_text": "", "category_id": 3, "user_id": 4},
                follow_redirects=True)
    response3 = client.get('/review/3/update')
...
def test_view_category(client, auth, app):
    auth.login()
    ...
    response2 = client.get('/category/3/descending_sort_rating/')
    ...
    response3 = client.get('/category/3/ascending_sort_rating/')
...
def test_add_review(client, app, auth):
    ...
    response = client.post('/add_review/',
                           data={"title": "test2", "rating": 5, "review_text": "gg", "category": "test_title"},
                           follow_redirects=True)
...
def test_delete_review(client, app, auth):
    auth.login()
    response = client.post('/review/4/delete/', follow_redirects=True)
    assert response.request.path == '/reviews/'

    "Проверка, что отзыв удален"
    with app.app_context():
        added_review = Reviews.query.filter_by(title='test2').first()

        assert added_review is None
